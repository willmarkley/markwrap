...
def run(command, cwd=None):
	...
	completed_process = subprocess.run(command, capture_output=True, text=True, cwd=cwd)
...
def quietRun(command, cwd=None):
	if not isinstance(command, type([])):
		logging.error("Command must be list: %s", command)
		raise RuntimeError()

	completed_process = subprocess.run(command, capture_output=True, cwd=cwd)
	if (completed_process.returncode != 0):
